...
def recurrent_neural_network(x):
	...
	# x = tf.split(0, n_chunks, x) #deprecated
	x = tf.split(axis=0, num_or_size_splits=n_chunks, value=x)

	#lstm_cell = rnn_cell.BasicLSTMCell(rnn_size,state_is_tuple=True) #deprecated
	# outputs, states = rnn.rnn(lstm_cell, x, dtype=tf.float32) #deprecated

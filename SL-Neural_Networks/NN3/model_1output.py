...
class Model:
	...
	def __init__(self, layers_dim): 
		...
		for i in range(len(layers_dim)-1):
			self.W.append(2*np.random.random((layers_dim[i],layers_dim[i+1]))-1)
	...
	def train(self, X, y, epochs=20000, lr=0.01, reg_lambda=0.01, print_loss=False):
		...
		# Forward propagation
			...
			v = np.atleast_2d(y) 
			loss = forward[len(forward)-1][2] - v.T
			#print(loss)
			dtanh=loss
			#print(dtanh)
			...
			if print_loss and epoch % 10000 == 0:
				print("Loss after iteration %i: %f" %(epoch, np.mean(np.abs(loss))))
				print(self.W)
